...
export function buildRightsChain(args: {
  ...
  const licensee = primary?.licensee?.trim() || operator || "Unknown operator";
  ...
  return [
    {
      layer: "itu",
      title: "ITU filing",
      holder: "ITU Space Network System",
      status: "stub",
      detail: "ITU SNS bulk data is not ingested in v0. A filed ITU network is the closest thing to a deed; this row is a placeholder until BR IFIC access lands.",
    ...
    },
    {
      ...
      detail: fccAuths.length > 0
        ? `${fccAuths.length} FCC authorization${fccAuths.length === 1 ? "" : "s"}${callSigns ? ` · ${callSigns}` : ""}.`
        : operator
          ? `UCS operator ${operator}. No FCC market-access row at this longitude (expected for many non-US administrations).`
    ...
    },
    {
      ...
      status: "stub",
      detail: "There is no public feed of transponder or payload sub-leases. This layer is reserved for operator-disclosed or future marketplace filings.",
