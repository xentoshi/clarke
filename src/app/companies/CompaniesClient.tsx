  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 mb-10">
        {verticals.map((v, i) => {
          const count = companies.filter((c) => c.vertical === v).length;
          const top = companies.filter((c) => c.vertical === v).slice(0, 2).map((c) => c.name);
          const isActive = vertical === v;
          return (
            <button key={v} onClick={() => setVertical(isActive ? "all" : v)}
              className={`text-left border rounded-xl p-4 relative overflow-hidden transition-colors ${
                isActive ? "border-zinc-600 bg-zinc-900/30" : "border-zinc-800 bg-zinc-900/10 hover:border-zinc-600 hover:bg-zinc-900/25"
              }`}>
              <div className="absolute top-0 left-0 right-0 h-[2px]" style={{ background: verticalAccent[v], opacity: isActive ? 1 : 0.6 }} />
              <div className="flex items-center justify-between mb-3">
                <span className="text-white/25 text-[10px] font-mono">{String(i + 1).padStart(2, "0")}</span>
                <span className="text-white/30 text-[10px] font-mono">{count}</span>
              </div>
              <div className={`text-sm font-semibold mb-1.5 leading-tight ${isActive ? "text-white" : "text-white/80"}`}>
                {verticalLabels[v]}
              </div>
              <p className="text-white/25 text-xs leading-relaxed line-clamp-2">
                {top.join(" · ")}{count > 2 ? ` +${count - 2}` : ""}
              </p>
            </button>
          );
        })}
